...
/**
 * One line of a document, with the match in it emphasised. The Mentions panel
 * draws the same rows for the same reason, so it takes this rather than
 * growing a second one that would drift from it.
 */
export function Line({ before, match, after }: Context) {
	return (
		<>
			{before}
...
export default function Search({
	...
	const results = useMemo(
		() => (corpus.kind === "ready" ? search(documents, query) : null),
		[corpus.kind, documents, query],
	);
